...
#us the data_detectors_meta to join with the stations metadata. The detectors that we have
data_detectors_meta_stations <- data_detectors_meta |>
  select(detector_id, stationid) |>
  left_join(stations, by = "stationid")
head(data_detectors_meta_stations)
